...
def process_image(x):
    with tf.name_scope('load_images'):
        if len(x.get_shape()) == 3:
            ...
            a_images = tf.expand_dims(preprocess(raw_input[:, :width // 2, :]), 0)
            b_images = tf.expand_dims(preprocess(raw_input[:, width // 2:, :]), 0)
        else:
            raw_input = tf.image.convert_image_dtype(x, dtype=tf.float32)
    ...
    def transform(image):
        ...
        r = tf.image.resize_images(
            r, [CROP_SIZE, CROP_SIZE], method=tf.image.ResizeMethod.AREA)
    ...
    # Tensor('batch:1', shape=(30, 256, 256, 3), dtype=float32) -> 1 batch size
...
def convert(image):
    return tf.image.convert_image_dtype(
        image, dtype=tf.uint8, saturate=True, name='output')  # output tensor
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        dest='input_folder',
        type=str,
        help='Model folder to import.')
    parser.add_argument(
        '--model-output',
        dest='output_folder',
        type=str,
        help='Model (reduced) folder to export.')
    parser.add_argument(
        "--batch-size",
        dest="batch_size",
        type=int,
    ...
    if args.batch_size == 1:
        x = tf.placeholder(tf.uint8, shape=(CROP_SIZE, 2 * CROP_SIZE, 3), name='image_tensor')
    else:
        x = tf.placeholder(tf.uint8, shape=(args.batch_size, CROP_SIZE, 2 * CROP_SIZE, 3), name='image_tensor')
    # output tensor
    y = generate_output(x)
